...
tbl <- data %>%
  ...
  modify_table_styling(
    ...
    footnote = "assessed with the ICD-11 screener (LIT), a self-report measure which items follow the ICD-11 diagnostic criteria for paraphilic disorders."
  ) %>%
  modify_table_styling(
    columns = label,
    rows = label %in% c("Highest education", "Years of education", "Marital status", "Employment status"),
    footnote = "information provided by the participant (SSQ-P)."
  ) %>%
  modify_table_styling(
    columns = label,
    rows = label %in% c("Conviction under the German Penal Code (StGB)", "Additional treatment", "Age", "Offense type"),
    footnote = "data extracted from case files by trained staff (SSQ-CF)."
  ) %>%
  modify_table_styling(
    columns = label,
    rows = label %in% c("Term of imprisonment (month)", "Duration of community supervision (month)", "Treatment ordered by court"),
    footnote = "information provided by supervision officer (SSQ-SO)."
  ) %>%
  modify_table_styling(
    columns = label,
    rows = label == "Static recidivism risk",
    footnote = "data extracted from case files by trained staff; since the Static-99 is only defined for hands-on offenders, we used the reduced Static-99 as mentioned by [LIT]; all staff member have been trained for the Static-99."
  ) %>%
  modify_table_styling(
    columns = label,
